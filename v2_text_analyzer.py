...
def get_word_frequency(words_list):
    freq = {}
...
def get_ngram_frequency(words_list, n_size):
    n_gram_freq = {}
    ...
    return sorted(n_gram_freq.items(), key=lambda item: item[1], reverse=True)


def calculate_readability_score(words_list, sentence_count):
...
sorted_freq = get_word_frequency(words_list)

top_n = int(input("How many top words do you need? "))

n_size = int(input("How many n-grams do you need? "))

sorted_ngram = get_ngram_frequency(words_list, n_size)

score = calculate_readability_score(words_list, sentence_count)
